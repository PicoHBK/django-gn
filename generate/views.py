...
class ConcatenatePromptsView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            with transaction.atomic():  # Aseguramos que todo se haga en una única transacción
                ...
                if not code.is_valid():
                    return Response({"error": "The code has no uses left."}, status=status.HTTP_406_NOT_ACCEPTABLE)

                # Verificamos si el código tiene un atributo 'tier'
                code_tier = code.tier
                if not code_tier:
                    return Response({"error": "The code does not have a valid tier."}, status=status.HTTP_400_BAD_REQUEST)
                ...
                if pose_name:
                    ...
                    if pose:
                        if not check_tier(pose.tier):
                            return Response({"error": "The code does not have the required tier to access this pose."}, status=status.HTTP_403_FORBIDDEN)
                ...
                if character_name:
                    try:
                        ...
                        if not check_tier(character.tier):
                            return Response({"error": "The code does not have the required tier to access this character"}, status=status.HTTP_403_FORBIDDEN)
                        ...
                        if skin_name:
                            ...
                            if skin:
                                if not check_tier(skin.tier):
                                    return Response({"error": "The code does not have the required tier to access this skin"}, status=status.HTTP_403_FORBIDDEN)
                                prompts.append(skin.prompt)
                            else:
                                return Response({"error": "Skin not found for the given character."}, status=status.HTTP_404_NOT_FOUND)
                        else:
                            return Response({"error": "Skin name is required."}, status=status.HTTP_400_BAD_REQUEST)
                ...
                if prompts:
                    ...
                    modified_data = modificar_json(file_path, concatenated_prompts, concatenated_prompts, image_type)
                    ...
                    if response.status_code == 200:
                        ...
                        if not code.use_code():
                            return Response({"error": "Failed to use the code. Please try again."}, status=status.HTTP_400_BAD_REQUEST)
                        ...
                        return Response({"error":"The AI is unavailable. Please try again later."}, status=status.HTTP_400_BAD_REQUEST)
                else:
                    # Si no hay recursos válidos, no usamos el código y devolvemos un error
                    return Response({"error": "No valid resources found or you do not have the required tier."}, status=status.HTTP_403_FORBIDDEN)
